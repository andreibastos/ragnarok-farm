...
async function seedContainers() {
    ...
    // Primeiro, inserir os itens dos containers na tabela items
    console.log('📦 Inserindo itens dos containers...');
    for (const container of containers) {
        await pool.query(`
            INSERT INTO items (id, name, type, description) 
            VALUES ($1, $2, 'Container', $3) 
            ON CONFLICT (id) DO NOTHING
        `, [container.itemId, container.name, `Container item: ${container.name}`]);
    }

    // Depois inserir os containers
    console.log('📦 Inserindo containers...');
    for (const container of containers) {
        await pool.query(`
